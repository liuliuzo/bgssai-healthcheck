...
5. **bgssai-bot** 对标 Grok Bot；BGSSAI 的全部产品应用可以托管给 Bot 直接操作。所有应用均可下载 Bot 安装包，全平台范围。连接各产品同时支持 MCP 个人令牌（PAT）和「用 Chat 登录」后下发 MCP。参考 grok-bot、openclaw、hermes-agent、grok-build、mycontext、deepseek-harness。对照这些项目，不要从 grok-build 等 grok 项目拷贝源码进产品仓；内核只走 bgssai-build 官方 fork。
